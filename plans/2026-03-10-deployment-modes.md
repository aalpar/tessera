...
func (m *mockS3Client) PutObjectIfNotExists(ctx context.Context, key string, body io.Reader) error {
	m.mu.Lock()
	defer m.mu.Unlock()
...
func (m *mockS3Client) ListCommonPrefixes(ctx context.Context, prefix string, delimiter string) ([]string, error) {
	m.mu.Lock()
	defer m.mu.Unlock()
...
// ErrKeyExists is returned by PutObjectIfNotExists when the key
// already exists (S3 returns 412 Precondition Failed).
var ErrKeyExists = errors.New("key already exists")

// s3API abstracts the S3 operations used by S3BlockStore and
// S3Transport. This allows testing with a mock client.
type s3API interface {
	PutObject(ctx context.Context, key string, body io.Reader) error
	// PutObjectIfNotExists writes only if key doesn't exist.
	// Returns ErrKeyExists on collision (S3 If-None-Match: *).
	PutObjectIfNotExists(ctx context.Context, key string, body io.Reader) error
	...
	// ListCommonPrefixes returns unique key prefixes using Delimiter,
	// without listing individual objects. Used for peer discovery.
	ListCommonPrefixes(ctx context.Context, prefix string, delimiter string) ([]string, error)
}
...
func (c *S3Client) PutObjectIfNotExists(ctx context.Context, key string, body io.Reader) error {
	ifNoneMatch := "*"
	_, err := c.client.PutObject(ctx, &s3.PutObjectInput{
		Bucket:      &c.bucket,
		Key:         &key,
	...
	if err != nil {
		// S3 returns 412 Precondition Failed when key exists.
		// The SDK wraps this as an API error with code "PreconditionFailed".
		var apiErr interface{ ErrorCode() string }
		if errors.As(err, &apiErr) && apiErr.ErrorCode() == "PreconditionFailed" {
			return ErrKeyExists
		}
		return fmt.Errorf("s3 put-if-not-exists %s: %w", key, err)
	}
	return nil
}
...
func (c *S3Client) ListCommonPrefixes(ctx context.Context, prefix string, delimiter string) ([]string, error) {
	input := &s3.ListObjectsV2Input{
		Bucket:    &c.bucket,
		Prefix:    &prefix,
		Delimiter: &delimiter,
	...
	for paginator.HasMorePages() {
		page, err := paginator.NextPage(ctx)
		if err != nil {
			return nil, fmt.Errorf("s3 list prefixes %s: %w", prefix, err)
		}
...
### Task 4b: S3 Integration Test Helper

Shared helper for integration tests that hit real S3. Uses the AWS
SDK credential chain (env → `~/.aws/credentials` → IAM role) — no
credentials in code. Tests skip automatically when the bucket env
var is unset. Cleanup uses `DeleteObjects` (batch delete) to remove
all test objects under an isolated per-test prefix.

**Files:**
- Create: `testutil_s3_integration_test.go` (`//go:build integration`)

**Step 1: Write the helper**

In `testutil_s3_integration_test.go`:
...
// s3IntegrationClient returns a real S3Client scoped to an isolated
// test prefix. Skips the test if TESSERA_S3_TEST_BUCKET is not set.
//
// The returned root prefix is unique per test invocation:
//
//	test/TestS3BlockStorePutGet/a3f7b2c1/
//
// The random run ID component prevents collision between concurrent
// runs of the same test. The test name component aids post-mortem
// inspection when cleanup fails and objects remain in S3.
//
// t.Cleanup deletes all objects under the root at test completion,
// so tests may write freely without manual teardown.
func s3IntegrationClient(t *testing.T) (*S3Client, string) {
	t.Helper()
	bucket := os.Getenv("TESSERA_S3_TEST_BUCKET")
	if bucket == "" {
		t.Skip("TESSERA_S3_TEST_BUCKET not set — skipping S3 integration test")
	}
	cfg, err := config.LoadDefaultConfig(context.Background())
	if err != nil {
		t.Fatalf("s3IntegrationClient: load AWS config: %v", err)
	}
	client := NewS3Client(s3.NewFromConfig(cfg), bucket)

	// Unique prefix: test name (sanitized) + random run ID.
	// t.Name() may contain "/" for subtests; replace with "_".
	runID := randomHex(4)
	name := strings.ReplaceAll(t.Name(), "/", "_")
	root := fmt.Sprintf("test/%s/%s/", name, runID)

	t.Cleanup(func() {
		ctx := context.Background()
		keys, err := client.ListObjects(ctx, root, "")
		if err != nil {
			t.Errorf("cleanup: list %s: %v", root, err)
			return
		}
		...
		for len(keys) > 0 {
			...
			if err := client.DeleteObjects(ctx, batch); err != nil {
				t.Errorf("cleanup: delete under %s: %v", root, err)
				return
			}
			keys = keys[len(batch):]
		}
...
```bash
git add testutil_s3_integration_test.go
git commit -m "Add S3 integration test helper — credential chain, isolated prefix, cleanup"
```

---

### Task 4c: S3BlockStore Integration Tests

Integration tests for `S3BlockStore` against real S3. Uses the
`s3IntegrationClient` helper from Task 4b.

**Files:**
- Create: `blockstore_s3_integration_test.go` (`//go:build integration`)

**Step 1: Write integration tests**

In `blockstore_s3_integration_test.go`:
...
func TestS3BlockStorePutGetIntegration(t *testing.T) {
	client, root := s3IntegrationClient(t)
	store := NewS3BlockStore(client, root+"blocks/")
	ctx := context.Background()

	chunk := newChunk([]byte("integration test data"))
	if err := store.Put(ctx, chunk.Hash, chunk.Data); err != nil {
		t.Fatal(err)
	}
...
func TestS3BlockStoreHasIntegration(t *testing.T) {
	client, root := s3IntegrationClient(t)
	store := NewS3BlockStore(client, root+"blocks/")
	ctx := context.Background()

	chunk := newChunk([]byte("exists check"))
	store.Put(ctx, chunk.Hash, chunk.Data)
	...
	has, err = store.Has(ctx, "nosuchblock")
	if err != nil || has {
		t.Fatalf("Has missing: got (%v, %v), want (false, nil)", has, err)
	}
}

func TestS3BlockStoreDeleteIntegration(t *testing.T) {
	client, root := s3IntegrationClient(t)
	store := NewS3BlockStore(client, root+"blocks/")
	ctx := context.Background()
...
func TestS3BlockStoreKeyLayoutIntegration(t *testing.T) {
	client, root := s3IntegrationClient(t)
	store := NewS3BlockStore(client, root+"blocks/")
	ctx := context.Background()

	chunk := newChunk([]byte("layout check"))
	store.Put(ctx, chunk.Hash, chunk.Data)

	// Verify sharding: {root}blocks/{ab}/{cdef...}
	expectedKey := root + "blocks/" + chunk.Hash[:2] + "/" + chunk.Hash[2:]
	keys, err := client.ListObjects(ctx, root+"blocks/", "")
	if err != nil {
		t.Fatal(err)
	}
	for _, k := range keys {
	...
	t.Errorf("expected key %q in S3, got: %v", expectedKey, keys)
}
```
...
```bash
TESSERA_S3_TEST_BUCKET=your-bucket go test -tags=integration -run 'Integration' -v ./...
```

**Step 3: Commit**

```bash
git add blockstore_s3_integration_test.go
git commit -m "Add S3BlockStore integration tests"
```
...
**Step 1: Write failing tests for S3Transport**

The `s3API` interface (including `ListObjects`, `ListCommonPrefixes`, and `PutObjectIfNotExists`) and all mock implementations are already defined in Task 3. S3Client adapter implementations are in Task 4.
...
// Publish uses PutObjectIfNotExists (If-None-Match: *) to prevent
// overwrites from stale seq counters or split-brain replica IDs.
//
// Receive discovers peers via ListCommonPrefixes (Delimiter="/"),
// which returns one entry per peer — O(peers), not O(total deltas).
...
type S3Transport struct {
	...
	// cursors tracks the last consumed S3 key per peer.
	// Keys after the cursor are new deltas to consume.
	cursors map[string]string // peer ID -> last consumed S3 key
}
...
func (t *S3Transport) Publish(delta []byte) error {
	...
	for {
		...
		err := t.client.PutObjectIfNotExists(ctx, key, bytes.NewReader(delta))
		if errors.Is(err, ErrKeyExists) {
			continue // seq collision — bump and retry
		}
		return err
	}
}
...
func (t *S3Transport) Receive() ([][]byte, error) {
	...
	// Discover peers using Delimiter — returns one CommonPrefix per
	// peer ("deltas/w1/", "deltas/w2/", ...) without listing
	// individual delta objects.
	peerPrefixes, err := t.client.ListCommonPrefixes(ctx, t.prefix, "/")
	if err != nil {
		return nil, fmt.Errorf("s3 transport discover peers: %w", err)
	}

	var result [][]byte
	for _, peerPrefix := range peerPrefixes {
		// Extract peer ID: "deltas/w1/" -> "w1"
		peer := strings.TrimPrefix(peerPrefix, t.prefix)
...
```bash
git add transport_s3.go transport_s3_test.go
git commit -m "Add S3Transport — metadata replication over S3 with peer discovery"
```

---

### Task 5b: S3Transport Integration Tests

Integration tests for `S3Transport` against real S3. Uses the
`s3IntegrationClient` helper from Task 4b. Two transports share the
same `root+"deltas/"` prefix, which mirrors real P2P usage where
peers share a single S3 bucket.

**Files:**
- Create: `transport_s3_integration_test.go` (`//go:build integration`)

**Step 1: Write integration tests**

In `transport_s3_integration_test.go`:
...
func TestS3TransportPublishReceiveIntegration(t *testing.T) {
	client, root := s3IntegrationClient(t)
	t1 := NewS3Transport(client, root+"deltas/", "w1")
	t2 := NewS3Transport(client, root+"deltas/", "w2")
...
func TestS3TransportCursorAdvancesIntegration(t *testing.T) {
	client, root := s3IntegrationClient(t)
	t1 := NewS3Transport(client, root+"deltas/", "w1")
	t2 := NewS3Transport(client, root+"deltas/", "w2")
...
func TestS3TransportNoSelfReceiveIntegration(t *testing.T) {
	client, root := s3IntegrationClient(t)
	t1 := NewS3Transport(client, root+"deltas/", "w1")

	t1.Publish([]byte("own-delta"))
...
func TestS3TransportPeerDiscoveryIntegration(t *testing.T) {
	client, root := s3IntegrationClient(t)
	t1 := NewS3Transport(client, root+"deltas/", "w1")
	t2 := NewS3Transport(client, root+"deltas/", "w2")
	t3 := NewS3Transport(client, root+"deltas/", "w3")

	t1.Publish([]byte("from-w1"))
	t2.Publish([]byte("from-w2"))

	deltas, err := t3.Receive()
	if err != nil {
		t.Fatal(err)
	}
	if len(deltas) != 2 {
...
```bash
git add transport_s3_integration_test.go
git commit -m "Add S3Transport integration tests"
```
